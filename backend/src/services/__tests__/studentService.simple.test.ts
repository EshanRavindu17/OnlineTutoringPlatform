...
describe('Student Service', () => {
  ...
  describe('addStudent', () => {
    it('should create a new student when user exists and no existing student', async () => {
      ...
      expect(mockPrismaClient.student.create).toHaveBeenCalledWith({
        data: {
          user_id: 'user123',
          points: 100,
          customer_id: 'cus_test123' // This comes from the Stripe mock
        }
      });
    ...
    it('should apply filters correctly', async () => {
      ...
      expect(mockPrismaClient.individual_Tutor.findMany).toHaveBeenCalledWith({
        where: {
          ...
          hourly_rate: { lte: 100 }, // min_hourly_rate is 0, so gte condition is not added
          rating: { gte: 4 },
      ...
      const mockSessions = [
        {
          ...
          i_tutor_id: 'tutor123',
          status: 'completed',
          Rating_N_Review_Session: [], // Add this property to avoid undefined error
          Individual_Tutor: {
            User: { name: 'Tutor 1' }
          }
        }
      ];
      mockPrismaClient.sessions.findMany.mockResolvedValue(mockSessions);

      // Act
      const result = await studentService.getAllSessionByStudentId('student123');

      // Assert
      expect(mockPrismaClient.sessions.findMany).toHaveBeenCalled();
      expect(result).toEqual([
        {
          ...mockSessions[0],
          reviewed: false // Rating_N_Review_Session.length is 0
        }
      ]);
    });
  });
  ...
  describe('getSlotsOfIndividualTutorById', () => {
    it('should return slots for a tutor', async () => {
      ...
      mockPrismaClient.free_Time_Slots.findMany.mockResolvedValue(mockSlots);

      // Act
      const result = await studentService.getSlotsOfIndividualTutorById('tutor123');

      // Assert
      // The function has additional filtering logic based on current date/time
      expect(mockPrismaClient.free_Time_Slots.findMany).toHaveBeenCalledWith({
        where: {
          i_tutor_id: 'tutor123',
          status: 'free',
          OR: expect.any(Array) // The function adds date filtering
        }
      });

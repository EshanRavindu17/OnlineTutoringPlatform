...
export default function EnhancedTutorUpload() {
  const [contentType, setContentType] = useState('video'); // 'video', 'article', 'quiz', 'assignment'
  ...
  const removeQuizTag = (tagToRemove) => {
    setQuiz(prev => ({
      ...prev,
      tags: prev.tags.filter(tag => tag !== tagToRemove)
    }));
  ...
  const removeAssignmentTag = (tagToRemove) => {
    setAssignment(prev => ({
      ...prev,
      tags: prev.tags.filter(tag => tag !== tagToRemove)
    }));
  ...
  const saveAssignment = async () => {
    ...
    try {
      ...
      formData.append('isPublic', assignment.isPublic);
      formData.append('allowLateSubmissions', assignment.allowLateSubmissions);
  ...
  // Video upload functions (keeping existing ones)
  const handleDrag = (e) => {
    e.preventDefault();
    e.stopPropagation();

...
import { Calendar, Clock, Plus, Edit2, Trash2, Save, X, CheckCircle, XCircle } from 'lucide-react';
...
const TutorAvailabilityCalendar: React.FC = () => {
  ...
  const handleEditSlot = (slot: TimeSlot): void => {
    setNewSlot({
      date: slot.date,
  ...
  const handleDeleteSlot = (slotId: number): void => {
    setTimeSlots(timeSlots.filter(slot => slot.id !== slotId));
  };

  const toggleAvailability = (slotId: number): void => {

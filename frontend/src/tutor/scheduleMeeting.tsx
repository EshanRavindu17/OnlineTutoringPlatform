...
import { Calendar, Clock, Video, X, Plus, PlayCircle, FileText, Star, Bell, Settings, Search } from 'lucide-react';
...
const OnlineMeetingSystem: React.FC = () => {
  ...
  const handleInputChange = (field: keyof NewMeeting, value: string | number): void => {
    setNewMeeting(prev => ({
      ...prev,
      [field]: value
    }));
  ...
  const startMeeting = (meetingId: string): void => {
  ...
  const renderUpcomingMeetings = (): React.JSX.Element => (
  ...
  const renderPastMeetings = (): React.JSX.Element => (
    <div className="space-y-4">
                  ...
                  <div className="flex items-center space-x-3 mb-2">
                    ...
                    <div className="flex items-center space-x-1">
                      {[...Array(5)].map((_, i) => (
                        <Star 
                          key={i} 
                          className={`h-4 w-4 ${i < (meeting.rating || 0) ? 'text-yellow-400 fill-current' : 'text-gray-300'}`} 
                        />
                      ))}
                    </div>
                  </div>
  ...
  const renderScheduleModal = (): React.JSX.Element => (

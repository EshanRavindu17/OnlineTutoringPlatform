...
import { Eye, EyeOff, Mail, Lock, User, ChevronRight, BookOpen, Users, Star, Shield, ArrowLeft } from 'lucide-react';
...
const SignupForm: React.FC<SignupFormProps> = ({ role = 'student' }) => {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      ...
      const { user: newUser }: { user: FirebaseUser } = await createUserWithEmailAndPassword(
        auth, 
        formData.email, 
        formData.password
      );
      ...
      if (response.data.success) {
        const destination = role === 'student' ? '/studentprofile' : '/tutorprofile';

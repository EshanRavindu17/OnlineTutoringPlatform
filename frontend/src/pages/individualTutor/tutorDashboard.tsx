    ...
    switch (material.type) {
      case 'document':
        if (material.url) {
          ...
          return (
            <div className="w-full h-96 p-4 border rounded bg-gray-50 overflow-auto">
      ...
      case 'link':
        return (
          <div className="p-8 text-center">
            <h3 className="text-lg font-medium mb-4">{material.name}</h3>
            <a 
              href={material.url} 
              target="_blank" 
...
const TutorDashboard: React.FC = () => {
  ...
  // Material type helper function
  const getMaterialType = (fileName: string): string => {
  ...
  // Material viewing handlers
  const handleViewMaterial = async (material: any, sessionId: string, materialIndex: number) => {
    try {
      if (currentUser?.uid) {
        // Call backend to get enhanced material content with Cloudinary URL
        const materialContent = await sessionService.getMaterialContent(
          currentUser.uid, 
          sessionId, 
          materialIndex.toString()
        );
        
        setSelectedMaterial(materialContent);
        setShowMaterialViewer(true);
      }
    } catch (error) {
      console.error('Error loading material content:', error);
      
      // Fallback to current local handling if backend fails
  ...
  const handleAddMaterial = async (sessionId: string, material: any) => {
    try {
      if (!currentUser?.uid) {
        alert('User not authenticated. Please log in and try again.');
        return;
      }
      await sessionService.addSessionMaterial(currentUser.uid, sessionId, material);
      // Refresh sessions to show new material
      await loadSessionsData();
    } catch (error) {
      console.error("Error adding material:", error);
    }
                  ...
                  return (
                    ...
                    >
                      ...
                      {(activeSessionTab === 'upcoming' || activeSessionTab === 'ongoing' || activeSessionTab === 'previous') && (
                          ...
                          {session.materials && session.materials.length > 0 ? (
                            <div className="space-y-2 mb-4">
                              {session.materials.map((material, index) => {
                                ...
                                let materialUrl: string | null;
                                let materialContent: string;
                                let materialSize: number | null;
                                let materialDescription: string | null;
                                ...
                                if (typeof material === 'string') {
                                  try {
                                    ...
                                    if (material.trim().startsWith('{') && material.trim().endsWith('}')) {
                                      ...
                                      materialName = parsedMaterial.name || 'Unnamed Material';
                                      materialType = parsedMaterial.type || 'text';
                                      materialUrl = parsedMaterial.url || null;
                                      materialContent = parsedMaterial.content || '';
                                      materialSize = parsedMaterial.size || null;
                                      materialDescription = parsedMaterial.description || null;
                                    } else {
                                      // It's a plain string material
                                  ...
                                  // It's already a Material object
                                  materialName = material.name;
                                  materialType = material.type;
                                  materialUrl = material.url || null;
                                  materialContent = material.content || '';
                                  materialSize = material.size || null;
                                        ...
                                        <div className="flex-1">
                                          ...
                                          {materialSize && (
                                            <div className="text-xs text-gray-500">
                                              {(materialSize / 1024 / 1024).toFixed(1)} MB
                                            </div>
                                          )}
                                        </div>
                                      </div>
                        ...
                        {materialUrl && (materialType === 'document' || materialType === 'presentation') && (
                          <button
                            onClick={() => handleViewMaterial(material, session.id, index)}
                            className="flex items-center px-3 py-1 bg-blue-100 text-blue-700 rounded-md hover:bg-blue-200 transition-colors text-xs"
                          >
                        ...
                        {materialUrl && materialType === 'video' && (
                          <button
                            onClick={() => handleViewMaterial(material, session.id, index)}
                        ...
                        {materialUrl && materialType === 'image' && (
                          <button
                            onClick={() => handleViewMaterial(material, session.id, index)}
                            className="flex items-center px-3 py-1 bg-purple-100 text-purple-700 rounded-md hover:bg-purple-200 transition-colors text-xs"
                          >
                            ...
                            View
                                    ...
                                    {materialType === 'text' && materialContent && (
                                      <div className="mt-2 p-2 bg-gray-50 rounded text-xs text-gray-600 border-l-2 border-gray-300">
                                        {materialContent.length > 100 ? `${materialContent.substring(0, 100)}...` : materialContent}
                                      </div>
                                    )}

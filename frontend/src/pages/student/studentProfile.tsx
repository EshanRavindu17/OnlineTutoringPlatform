...
// Separate Rating Modal Component to prevent re-rendering issues
...
const RatingModal: React.FC<RatingModalProps> = React.memo(({
  isOpen,
  selectedSession,
  rating,
  review,
...
const StudentProfile: React.FC = () => {
  ...
  // Rating and Review Functions - Optimized with useCallback to prevent unnecessary re-renders
  const openRatingModal = useCallback((session: any) => {
  ...
  const handleRatingChange = useCallback((ratingValue: number) => {
    setRating(ratingValue);
  }, []);

  const handleReviewChange = useCallback((reviewValue: string) => {
  ...
  const submitRating = useCallback(async () => {
    ...
    if (!studentId) {
      showToast('Student ID not available. Please try again.', 'error');
      return;
    }
    ...
    try {
      // Call the actual API to submit rating and review
      await createAndReview(studentId, selectedSession.session_id, rating, review);
  ...
  }, [selectedSession, rating, review, studentId, showToast, closeRatingModal]);
    ...
    <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6 mb-8">
      ...
      {tutorsLoading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
          ...
          <button onClick={() => navigate(`/tutor-profile/${tutor.id}`)} key={tutor.id} className="w-full text-left">
                  ...
                  <div key={report.id} className="border border-gray-200 rounded-xl p-6 hover:shadow-lg transition-all duration-300">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4">
                      <div className="flex items-start space-x-4">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-gray-500">Submitted: {new Date(report.submitted_date).toLocaleDateString()}</p>
                        </div>
                      </div>
                      <div className="mt-4 sm:mt-0">
                        <span className={`px-4 py-2 rounded-full text-sm font-bold ${
                          report.status === 'under_review' ? 'bg-yellow-100 text-yellow-800' :
                          report.status === 'solve' ? 'bg-green-100 text-green-800' :
                    ...
                    <div className="mb-4">
                      <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                        ...
                        <p className="text-red-700 font-medium">{report.reason}</p>
                      </div>
                    </div>
                    ...
                    {report.response && (
                      <div className="mb-4">
                        ...
                        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                          <p className="text-blue-800 leading-relaxed">{report.response}</p>
                          {report.resolve_date && (
                            <p className="text-blue-600 text-sm mt-2 font-medium">
                              Resolved on: {new Date(report.resolve_date).toLocaleDateString()}
                            </p>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="flex items-center text-sm text-gray-500 space-x-4">
                        ...
                        {report.resolve_date && (
                          <span className="flex items-center">
                            ...
                            {report.status}: {new Date(report.resolve_date).toLocaleDateString()}
                          </span>
                        )}
                      </div>
                    </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  ...
                  <div className="bg-white rounded-lg p-4 text-center">
                    <div className="text-2xl font-bold text-green-600">
                      {submittedReports.filter(r => r.status === 'solve' || r.status === 'Closed').length}
                    </div>
                    <div className="text-gray-600 text-sm">Resolved</div>
                  </div>
                  <div className="bg-white rounded-lg p-4 text-center">
                    <div className="text-2xl font-bold text-yellow-600">
                      {submittedReports.filter(r => r.status === 'under_review').length}
                    </div>

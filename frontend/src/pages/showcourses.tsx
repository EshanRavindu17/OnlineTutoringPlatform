...
import { Search, Filter, Star, Clock, Users, BookOpen, Grid, List, ArrowUpDown, Heart, ShoppingCart, Play } from 'lucide-react';
...
const ViewAllCoursesPage = () => {
  ...
  const categories: string[] = ['Technology', 'Mathematics', 'Languages', 'Business', 'Arts & Design', 'Science', 'Health & Fitness', 'Music'];
  const levels: string[] = ['Beginner', 'Intermediate', 'Advanced', 'All Levels'];
  ...
  const ratings: number[] = [4.5, 4.0, 3.5, 3.0];
  ...
  const toggleFavorite = (courseId: number): void => {
  ...
  const toggleCart = (courseId: number): void => {
  ...
  const CourseCard = ({ course, isListView = false }: CourseCardProps) => (
